...
object NetUtil {
    ...
    init {
        wifiManager =
            SKExtension.getApplication().getSystemService(Context.WIFI_SERVICE) as WifiManager
    }
    ...
    private fun openWifi(): Boolean {
        if (!wifiManager.isWifiEnabled) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                //请用户手动打开wifi
                ToastUtils.showShort("请前往设置打开wifi")
                return false
            } else {
